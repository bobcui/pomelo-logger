...
function initReloadConfiguration(filename, reloadSecs) {
	...
	configState.timerId = setInterval(reloadConfigurationAsync, reloadSecs * 1000);
};
...
function loadConfigurationFileAsync(filename, callback) {
    fs.readFile(filename, 'utf8', function(err, data) {
        if (!!err) {
            console.error('read log config file %s fail err=%s', filename, err.stack)
            callback(null)
        }
        else {
            var config
            try {
                ...
                console.log('reload log config file %s mtime=%s', filename, configState.lastMTime.getTime())
                console.log(config)
            }
            catch (e) {
                console.error('reload log config file %s failed mtime=%s err=%s', filename, configState.lastMTime.getTime(), e.stack)
            }
            callback(config)
        }
    })
