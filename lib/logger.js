...
function getMTimeAsync(filename, callback) {
    fs.stat(filename, function(err, stats){
        if (!!err) {
            console.error('stat log config file %s fail err=%j', path, err)
            callback(null)
        }
        else {
...
function loadConfigurationFileAsync(filename, callback) {
    fs.readFile(filename, 'utf8', function(err, data) {
        if (!!err) {
            console.error('read log config file %s fail err=%j', path, err)
            callback(null)
        }
        else {
            var config = JSON.parse(data)
            console.log('reload log config file %s mtime=%s', path, configState.lastMTime.getTime())
            console.log(config)
            callback(data)
        }
    })
